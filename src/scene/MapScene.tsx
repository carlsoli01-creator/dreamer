...
export default function MapScene({ map }: Props) {
  const centerOffset: [number, number] = [map.size.width / 2, map.size.depth / 2];
  const camDistance = Math.max(map.size.width, map.size.depth) * 0.85;
  ...
  return (
    <Canvas
      ...
      camera={{ position: [camDistance * 0.7, camDistance * 0.55, camDistance * 0.7], fov: 45, near: 0.1, far: 800 }}
    >
      ...
      {/* base ground beyond bounds (helps grounding visually) */}
      ...
      {/* map ground tile (slightly raised for distinction) */}
      ...
      <Suspense fallback={null}>
        <SceneContent map={map} centerOffset={centerOffset} />
      </Suspense>
      ...
      <OrbitControls
        ...
        minDistance={10}
        maxDistance={camDistance * 1.6}
      />
    </Canvas>
  );
}

function SceneContent({ map, centerOffset }: { map: MapData; centerOffset: [number, number] }) {
  return (
    <>
      <Boundary map={map} centerOffset={centerOffset} />
      {map.outdoorZones.map((z, i) => (
        <OutdoorZone key={`z-${i}`} zone={z} centerOffset={centerOffset} map={map} />
      ))}
      {map.buildings.map((b) => (
        <Building key={b.id} building={b} centerOffset={centerOffset} map={map} />
      ))}
      <SpawnMarkers spawns={map.spawnPoints} centerOffset={centerOffset} />
      {map.extractionZones.map((e, i) => (
        <ExtractionMarker key={`e-${i}`} zone={e} centerOffset={centerOffset} />
      ))}
      {map.soundTraps?.map((t, i) => (
        <SoundTrapMarker key={`t-${i}`} trap={t} centerOffset={centerOffset} />
      ))}
    </>
  );
}
  ...
  return (
    <>
      ...
      <hemisphereLight args={[
        isSnow ? '#cad3df' : '#3a4858',
        isPlatform ? '#020202' : '#080808',
        isSnow ? 0.7 : 0.45,
      ]} />
      <directionalLight
